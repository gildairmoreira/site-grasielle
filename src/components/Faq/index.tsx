...
const Faq: React.FC<FAQProps> = ({ faqs }) => {
  ...
  return (
    <section id="faq" className="bg-white__fundo-100 flex flex-col items-center pb-16">
          ...
          <div key={faq.id} className="transition">
            ...
            >
              {activeIndices.includes(faq.id) ? (
                <IoMdArrowDropupCircle className="text-gold__principal-500 transition" size={20} />
              ) : (
                <IoMdArrowDropdownCircle className="text-black transition" size={20} />
              )}
              <h3
                className={`pl-2 text-lg font-semibold ${
                  activeIndices.includes(faq.id) ? 'text-gold__principal-500' : ''
                }`}
              >
                {faq.title}
              </h3>
            ...
            <div
              className={`mt-2 text-gray-600 transition-opacity ${
                activeIndices.includes(faq.id) ? 'opacity-100' : 'hidden opacity-0'
              }`}
            >
              {faq.description}
            </div>
